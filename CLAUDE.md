...
**Authentication:** Auth0 JWT Bearer in non-dev environments. Local dev bypasses Auth0 entirely via `DevAuthHandler` (API, swapped in when `IsDevelopment()`) and `DevAuthContext` (frontend, when `import.meta.env.DEV`). In production the frontend does **not** use the `@auth0/auth0-react` SDK — `AuthContext.tsx` drives `Auth0LockPasswordless` directly (email code + Google OAuth), stores the token in `localStorage`, and decodes the `id_token` JWT client-side for user claims, because a custom API audience breaks Auth0's `/userinfo` endpoint. Users are auto-registered on first `/api/auth/me` call. `IsAdmin` is set directly in the database — there are no Auth0 roles. `AuthContext`'s `getAccessTokenSilently` must always **resolve** (to `''` when there's no session), never reject — `App.tsx` wires it as the unconditional token-getter for every `api.*` call via `client.ts`'s `request<T>`, which has no `.catch` around the token fetch, so a rejecting implementation breaks *all* anonymous API access app-wide (this happened in production: it silently broke the `/unsubscribe` flow, then was caught and fixed while building the anonymous-access DCI pages below — `DevAuthContext`'s dev-mode implementation always resolved, which is why local testing never surfaced it).
...
**Public DCI data:** `IDciPublicService`/`DciPublicService` + `PublicDciController` (route prefix `api/dci`, zero `[Authorize]` anywhere in the file) expose read-only, fully anonymous access to real-world DCI tour data — standings/schedule/scores/recap — the app's first anonymous-access API surface. Explicitly distinct from `StandingsService` above: this reads `ScoreEntity`/`ShowEntity` directly for a corps' actual competition results, not the fantasy-league scoring built on `ComputedScoreEntity`. A corps' real total score is a lookup (the standalone `Caption.Total` row per `(CorpsId, ShowId)`), not an aggregation. Ranks are always computed from real score values at read time, never from `ScoreEntity`'s scraped `TotalRank`/`RepertoireRank`/`PerformanceRank` fields. Every by-ID endpoint filters on the season's `IsPublished` flag, matching `GetCurrentSeasonAsync`'s own tiered resolution (Active → most recently-ended Completed → most recent Upcoming).
